...
app.get('/send-notification', (req, res) => {
  ...
  const notificationPayload = {
      title: title,
      body: body,
      icon: '/assets/bunny.png', // Fixed icon
  };
...
async function sendMessageToWebhook(message) {
  if (!message.includes("The Easter Bunny just left **Easter Bunny's Workshop, Easter Island, Chile**")) {
  const webhookUrl =
...
app.use((req, res, next) => {
  ...
  const excludedSubdomains = ['temp', 'developer', 'bunny', 'status', 'api'];
  ...
  if (!excludedSubdomains.includes(subdomain)) {
    res.redirect(301, 'https://bunny.master-trackers.xyz' + req.originalUrl);
...
async function isSameMonthAsCountdown() {
  ...
  try {
    ...
    if (lines.length > 4) {
      ...
      const isBeforeDec25 = utcPlus14Date.getUTCDate() < 31;
...
app.get("/getcountdowndate", (req, res) => {
  ...
  const searchString = "1\t1\t1\tBunny's House\tp";
...
function sendNextCity() {
  if (isTrackerStarted && currentIndex < cities.length) {
    ...
    sendMessageToWebhook(
      "Satellites are saying that the Easter Bunny just left **" +
      city.city +
      ", " +
      city.country +
      "**"
...
app.get('/getlastcity', (req, res) => {
  const city = lastCity || "Easter Bunny's Workshop, Easter Island, Chile";
...
if (!preventCacheParam || (currentTimestamp - preventCacheParam > 5 * 1000)) {
...
app.get("/mastermap", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "neutral", "mastermap.html"));
});
app.get("/mastermap/api/holidays.json", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "neutral", "holidays.json"));
