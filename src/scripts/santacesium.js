...
const deltaHeading = normalizeAngle(targetHeading );
...
var santa2 = viewer.entities.add({
  ...
  id: "santa2",
  model: {
    uri: '/assets/bunny2.gltf',
    scale: 100.1 // Smaller scale
  },
          ...
          TweenLite.to({ progress: prog1 }, actualtime, {
            ...
            onUpdate: function () {
              if (isheadingrn === false) {
                ...
                heading = geodesic5.startHeading; // Heading in radians
...
const adjustedHeading = normalizedTargetHeading;
...
function myFunctio2() {
  ...
  santa2.orientation = Cesium.Transforms.headingPitchRollQuaternion(
    ...
    new Cesium.HeadingPitchRoll(heading, roll, pitch)
  );
...
if (cityInfo.city === "International Space Station") {
  height5555 = 300100
    ...
    var iconImage = isYouTubeLink ? '/assets/camera.png' : '/assets/basket.png';
  ...
  try {
    ...
    if (latitude && longitude) {
      ...
      if (timeUntilArrival >= (currenttimeunixsec/1000)) {
        ...
        arrivalInfoElement.innerText = `The Easter Bunny will arrive in: ${formatArrivalTime(timeUntilArrival)}`;
      } else {
        ...
        arrivalInfoElement.innerText = `The Easter Bunny arrived at: ${formattedArrivalTime}`;
      }
setInterval(() => {
...
function setCityImage(nextCity2) {
  if (debounceTimeout2 === true) return; // Debounce check with last city
  debounceTimeout2 = true
  ...
  if (imageData[nextCity2] && imageData[nextCity2].length > 0) {
    ...
    let cityParts = nextCity2.split(','); // Split by commas
    let formattedCity = cityParts.slice(0, 2).join(', '); // Get first two parts and join them
    document.getElementById('flag3title').innerText = formattedCity;    
    document.getElementById('lastCityInfo3').innerHTML = `© <a target="_blank" href="${randomImage.attribution}">Attribution</a>`
  } else {
    ...
    document.getElementById('flag3title').innerText = "";
...
setInterval(() => {
  debounceTimeout2 = false;; // Reset debounce after 20 seconds
}, 20000);
...
console.log(data.nextCity.unixarrivalarrival-(currenttimeunixsec/1000))
      ...
      if (data.timeLeft <= parseInt(data.currentCity.Arrival_Stoppage_Time)) {
        ...
        santaOverlay.billboard.image = "/assets/bunny.png"
        moving = true;
        stopped = true;
      ...
      setCityImage(nextCity2 + ", " + data.nextCity.country);
      if (prog1set === false) {
    ...
    if (santa && santa.position) {
      ...
      if (position) {
        ...
        setTimeout(() => {
          ...
          const eyeOffsetValue = eyeOffsetParam ? parseFloat(eyeOffsetParam) * 1000 : 12000; // Multiply by 1000 or default to 1200
