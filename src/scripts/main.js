      ...
      try {
        ...
        if (latitude && longitude) {
          ...
          if (timeUntilArrival >= 0) {
            arrivalInfoElement.innerText = `The Easter Bunny will arrive in: ${formatTimeUntilArrival(timeUntilArrival)}`;
          } else {
            arrivalInfoElement.innerText = `The Easter Bunny arrived ${formatTimeUntilArrival(-timeUntilArrival)} ago`;
          }
        } else {
...
console.log(nextLatLng)
      ...
      if (data.timeLeft <= parseInt(data.currentCity.Arrival_Stoppage_Time)) {
        ...
        if (santaMarker.options.icon.options.iconUrl !== '/assets/bunny.png') {
          santaMarker.setIcon(L.icon({
            iconUrl: '/assets/bunny.png', // Change to delivering.gif
    ...
    function addBasket(cityInfo) {
      ...
      const basketMarker = L.marker([cityInfo.latitude, cityInfo.longitude], {
        icon: L.divIcon({
          ...
          iconSize: [50, 50], // size of the icon
          iconAnchor: [25, 25], // point of the icon which will correspond to marker's location
          html: '<img src="/assets/basket.png" width="50px" height="50px">'
        })
      }).addTo(map);
